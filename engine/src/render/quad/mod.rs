...
pub use types::{QuadInstance, VisParams};

use pipeline::{make_pipeline, stencil_read_state};
use shader::SHADER;
...
impl Renderer {
    pub fn new(device: &wgpu::Device, format: wgpu::TextureFormat) -> Self {
        ...
        let instance_buf_enemy = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("instances (enemy)"),
            size: MAX_INSTANCES * std::mem::size_of::<QuadInstance>() as u64,
            usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });
        ...
        let pipeline_enemy = make_pipeline(
            device,
            &pipeline_layout,
            &shader,
            vertex_buffers,
    ...
    fn write_uniforms(
        ...
        screen_w: f32,
        screen_h: f32,
        vis: Option<&VisParams>,
    ) {
        let u = match vis {
            Some(v) => Uniforms {
                screen_size: [screen_w, screen_h],
                player_pos: v.player_pos,
                player_dir: v.player_dir,
                cos_half_fov: v.cos_half_fov,
                cone_range: v.cone_range,
    ...
    pub fn draw(
        ...
        self.write_uniforms(queue, screen_w, screen_h, None);
        if !instances.is_empty() {
    ...
    pub fn draw_masked(
        ...
        self.write_uniforms(queue, screen_w, screen_h, None);
        if !instances.is_empty() {
            queue.write_buffer(&self.instance_buf_enemy, 0, bytemuck::cast_slice(instances));
        }
    ...
    pub fn draw_scene(
        ...
        self.write_uniforms(queue, screen_w, screen_h, Some(vis));
        if !instances.is_empty() {
            queue.write_buffer(&self.instance_buf, 0, bytemuck::cast_slice(instances));
        }
        self.draw_pass(
