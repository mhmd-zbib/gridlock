...
pub fn editor_texts(sw: f32, sh: f32, editor: &Editor) -> Vec<TextSection> {
    ...
    let prop_info = match editor.selected_prop_asset() {
        ...
        None => "Prop Id: (none found in assets/configs/props/*.json)".to_string(),
    };
    let prop_assets_line = if editor.prop_assets().is_empty() {
    ...
    let floor_info = match editor.selected_floor_asset() {
        Some(asset) => format!(
            "Floor Id: {} ({}/{})  {:.2}x{:.2}",
            asset.id,
            editor.selected_floor_asset_index() + 1,
            editor.floor_assets().len(),
            asset.width,
            asset.height
        ),
        None => "Floor Id: (none found in assets/configs/floors/*.json)".to_string(),
    };
    let floor_assets_line = if editor.floor_assets().is_empty() {
        "Floor Ids: (none)".to_string()
    } else {
        let mut labels: Vec<String> = editor
            .floor_assets()
            .iter()
            ...
            .take(6)
            .map(|(idx, asset)| {
                if idx == editor.selected_floor_asset_index() {
                    format!("[{}]", asset.id)
                } else {
                    asset.id.clone()
                }
            ...
            .collect();
        if editor.floor_assets().len() > 6 {
            labels.push(format!("+{}", editor.floor_assets().len() - 6));
        }
        format!("Floor Ids: {}", labels.join("  "))
    };
    let (asset_info, assets_line) = if editor.tool == Tool::Floor {
        (floor_info, floor_assets_line)
    } else {
    ...
    let stats = format!(
        "Enemies: {}  Targets: {}  Walls: {}  Breakables: {}  Props: {}  Floors: {}  Map: {}  Zoom: {:.2}x",
        editor.level.enemies.len(),
        editor.level.target_enemies.len(),
        solids,
        breakables,
        editor.level.props.len(),
        editor.level.floors.len(),
