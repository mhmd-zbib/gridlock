...
use net::{AnyPacket, ClientPacket, ConnectAck, ConnectResult, NetSocket, decode_rotation, encode};
...
impl ServerState {
    fn new() -> Self {
        Self { sessions: HashMap::new(), next_player_id: 1 }
    }
}
...
async fn main() {
    ...
    loop {
        ...
        let engine_input = player_input
            .map(|(_, pkt)| client_input_to_engine(
                &pkt,
        ...
        if !addrs.is_empty() {
            let snapshot = build_snapshot(tick, bullets, &game);
            println!(
...
async fn handle_packet(
    ...
    match packet {
        AnyPacket::ConnectRequest(req) => {
            enum Decision { ReAck(u16), NewPlayer(u16), Full }
            ...
            let decision = {
                ...
                if let Some(s) = st.sessions.get(&addr) {
                    ...
                    st.sessions.insert(addr, Session {
                        player_id: pid,
            ...
            match decision {
                Decision::Full => {
                    let _ = socket.send_to(&AnyPacket::ConnectAck(ConnectAck {
...
fn build_snapshot(tick: u32, bullets: Vec<BulletEvent>, game: &Game) -> ServerPacket {
    ServerPacket {
        tick,
        timestamp: 0,
        me: SelfState {
            ...
            movement_state: net::MovementState(0),
        },
        players: vec![],
        bullets,
        sounds: vec![],
