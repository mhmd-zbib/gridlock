...
impl LevelSelect {
    pub fn new() -> Self {
        let levels = scan();
        let s = Self { levels, selected: 0, prev_up: false, prev_down: false };
        s.print_list();
        s
    }
    ...
    pub fn navigate(&mut self, up: bool, down: bool) {
        if up && !self.prev_up && !self.levels.is_empty() {
            if self.selected == 0 { self.selected = self.levels.len() - 1; }
            else                  { self.selected -= 1; }
            println!("[select] ↑  [{}] {}", self.selected, self.levels[self.selected].display());
        }
...
fn scan() -> Vec<PathBuf> {
    let Ok(dir) = std::fs::read_dir("levels") else { return Vec::new(); };
